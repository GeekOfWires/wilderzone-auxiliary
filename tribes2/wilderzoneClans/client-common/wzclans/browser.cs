...
//  - Adapted from the TribesNext t2-scripts browser.cs (TribesNext Project, 2011-2012).
//  - All community identifiers renamed: $TribesNext::Community:: -> $WZClans::Community::
//    (including the stray $TribesNext::Browser::CertRefreshSch, now
//    $WZClans::Community::Browser::CertRefreshSch) and tn_community_* -> wzClans_*.
//    The $T2CSRI:: globals are intentionally NOT renamed: the engine's t2csri
//    clan-certificate streaming reads them.
//  - TRANSPORT SPLIT: this shared file no longer creates the WZClansBrowserInterface
//    TCPObject and no longer builds raw HTTP request text. Requests are queued as
//    (path+query, POST payload) pairs in $BrowserRequestQueue / $BrowserPayloadQueue
//    and handed to the flavor hook wzClansHttpRequest("browser", %path, %payload).
//    Responses come back through wzClans_browser_onLine(%line) /
//    wzClans_browser_onDisconnect(), which the flavor calls via the shared
//    wzClansOnLine/wzClansOnDisconnect dispatchers after stripping HTTP headers.
//  - The cert-refresh scheduling now uses the flavor hook wzClansEpoch() instead of
//    rubyEval Time.now().to_i.
//  - $LoginCertificate reads now go through the flavor hook wzClansGetLoginCertificate().
//  - Assumption kept from the original: the LaunchTabView/EmailGui/
//    TribeandWarriorBrowserGui GUI objects referenced in the "CEC" case are stock
//    Dynamix launch-tab GUI elements and must exist on the client.
...
$WZClans::Community::Browser::Active = 0;

// Wilderzone: the flavor transport calls this (via wzClansOnDisconnect) when a
// browser response completes. The original tested %this.primed to detect a
// completely absent response; the flavor now consumes the HTTP headers, so the
// shared code instead tracks whether any body line arrived for the request.
function wzClans_browser_onDisconnect()
{
	if (!($WZClans::Community::Browser::GotResponse))
	{
		...
		if (!$WZClans::Community::Browser::Firewalled)
		{
			$WZClans::Community::Browser::Firewalled = 1;
	...
	$WZClans::Community::Browser::Active = 0;
	wzClans_Browser_executeNextRequest();
}
...
function wzClans_browser_onLine(%line)
{
	$WZClans::Community::Browser::GotResponse = 1;
	...
	if (getSubStr(%line, 0, 4) $= "ERR:")
	{
		...
		%detail = trim(getSubStr(%line, 4, strlen(%line)));
		// Wilderzone: the community server's cert issuer is disabled (its
		// signer's validity period has expired) - this is EXPECTED now, so log
		// it once per game session instead of blocking input with a modal
		if (strStr(%detail, "validity period") >= 0)
		{
			if (!$WZClans::Community::Browser::SignerWarned)
			{
				$WZClans::Community::Browser::SignerWarned = 1;
				warn("WZClans: community server cannot issue certificates:" @ %detail);
				warn("WZClans: cert-based clan tags are unavailable; session, browser and mail are unaffected.");
			}
		}
		else
			schedule(500, 0, MessageBoxOK, "SYSTEM ERROR", %detail);
	}
	...
	switch$ (%message)
	{
		...
		case "ERR":
			...
			{
				// Wilderzone: one modal per game session. Server-side issuer
				// failures (e.g. "Signer validity period has expired" - the
				// DCE can no longer mint certs) would otherwise nag the user
				// with a popup on every retry.
				if (!$WZClans::Community::Browser::ErrorShown)
				{
					$WZClans::Community::Browser::ErrorShown = 1;
					schedule(500, 0, MessageBoxOK, "ERROR", getFields(%line, 2));
				}
				else
					warn("WZClans browser error: " @ getFields(%line, 2));
			}
		...
		case "DCE":
			...
			%dceIssued = getField(%dceCert, 2);
			%dceExpire = getField(%dceCert, 3);
			// Wilderzone: log the validity window so signer/issuer problems are
			// diagnosable from the console (dates are UTC)
			echo("WZClans: DCE cert '" @ getField(%dceCert, 0) @ "' (#" @ getField(%dceCert, 1) @ ") issued " @ wzClans_mailui_epochToDate(%dceIssued) @ ", expires " @ wzClans_mailui_epochToDate(%dceExpire) @ "; local time is " @ wzClans_mailui_epochToDate(wzClansEpoch()));
			// Wilderzone: never keep an expired DCE cert. Presenting one gets the
			// client KICKED by game servers ("DCE is not valid for the current
			// time period." in t2csri/serverSideClans.cs). Field 3 is the expiry
			// epoch.
			if (%dceExpire !$= "" && %dceExpire < wzClansEpoch())
			{
				warn("WZClans: community server sent an expired DCE certificate (expired " @ wzClans_mailui_epochToDate(%dceExpire) @ ") - ignoring it.");
				return;
			}
			%index = getField(%dceCert, 1);
			$T2CSRI::ClientDCESupport::DCECert[%index] = %dceCert;
		case "CEC":
			// Wilderzone: the original stored the cert BEFORE checking expiry -
			// with an expired cert in $T2CSRI::CommunityCertificate, the stock
			// client code streams it to game servers and the client gets KICKED
			// ("Community cert has expired. Get a fresh one from the DCE.").
			// Only store certs that are actually still valid.
			%cec = collapseEscape(getField(%line, 1));
			%expire = getField(%cec, 2);
			// log the validity window (dates are UTC) so issuer-side problems
			// are diagnosable from the console
			echo("WZClans: community cert issued " @ wzClans_mailui_epochToDate(getField(%cec, 1)) @ ", expires " @ wzClans_mailui_epochToDate(%expire) @ "; local time is " @ wzClans_mailui_epochToDate(wzClansEpoch()));
			// seconds of remaining lifetime (minus a 60s safety margin); integer
			// subtraction, exact in TorqueScript
			%remaining = %expire - wzClansEpoch() - 60;
			if (%remaining > 0)
			{
				$T2CSRI::CommunityCertificate = %cec;
				if (isEventPending($WZClans::Community::Browser::CertRefreshSch))
					cancel($WZClans::Community::Browser::CertRefreshSch);
				$WZClans::Community::Browser::CertRefreshSch = schedule(1000 * %remaining, 0, wzClans_Browser_request_cert);
			}
			else
			{
				$T2CSRI::CommunityCertificate = "";
				if (!$WZClans::Community::Browser::ExpiredCertWarned)
				{
					$WZClans::Community::Browser::ExpiredCertWarned = 1;
					schedule(500, 0, MessageBoxOK, "ERROR", "Received expired certificate from community server (expired " @ wzClans_mailui_epochToDate(%expire) @ " UTC). Is your computer's clock set correctly?");
				}
				else
					warn("WZClans: received an expired community certificate (expired " @ wzClans_mailui_epochToDate(%expire) @ ") - not storing it.");
			}
		...
		case "CLAN":
			...
			%clan = wzClans_browser_getClanProfile(%id);
			if (%var $= "NAME")
			{
				// wipe the players so membership doesn't look screwed up if it shrinks
				wzClans_browser_wipePlayers(%clan);
			...
			else if (%var $= "DATE")
			{
				...
				%clan.date = wzClans_mailui_epochToDate(getField(%line, 3));
			}
			else if (%var $= "PICT")
			{
		...
		case "SEARCH":
			...
			%idx = getField(%line, 2) + 0;
			%answer = wzClans_util_extractPlayer(%line, 3);
		...
		case "PLAYER":
			...
			if (%var $= "NAME")
			{
				wzClans_browser_wipeMemberships(%player);
			...
			{
				// convert epoch to human readable
				%player.date = wzClans_mailui_epochToDate(getField(%line, 3));
			}
			else if (%var $= "SITE")
			{
		...
		case "CLAN_RECRUITING":
			...
			wzClans_browser_getClanProfile(%id).lastRefresh = 0;
			wzClans_browser_clan_view(%id);
		...
		case "RENAMED":
			...
			wzClans_browser_user_view(getField(wzClansGetLoginCertificate(), 1));
			wzClans_browser_request_cert();
...
function wzClans_browser_wipeMemberships(%this)
{
	%count = %this.mcount;
	for (%i = 0; %i < %count; %i++)
	{
...
function wzClans_browser_wipePlayers(%this)
{
	%count = %this.pcount;
	for (%i = 0; %i < %count; %i++)
	{
...
function wzClans_browser_getPlayerProfile(%guid)
{
...
function wzClans_browser_getClanProfile(%id)
{
...
function wzClans_browser_initQueue()
{
	// initialize message vectors to handle queuing requests to the remote system
	...
	if (isObject($BrowserPayloadQueue))
		$BrowserPayloadQueue.delete();
	$BrowserPayloadQueue = new MessageVector();
...
wzClans_browser_initQueue();

function wzClans_browser_processRequest(%request, %payload)
{
	...
	%path = $WZClans::Community::BaseURL @ $WZClans::Community::BrowserScript @ %request;

	$BrowserRequestQueue.pushBackLine(%path);
	...
	if (%payload $= "")
		%payload = " ";
	$BrowserPayloadQueue.pushBackLine(%payload);

	if (!$WZClans::Community::Browser::Active)
		wzClans_browser_executeNextRequest();
}

function wzClans_browser_executeNextRequest()
{
	if ($BrowserRequestQueue.getNumLines() <= 0)
		return;

	%path = $BrowserRequestQueue.getLineText(0);
	$BrowserRequestQueue.popFrontLine();
	%payload = $BrowserPayloadQueue.getLineText(0);
	$BrowserPayloadQueue.popFrontLine();
	if (%payload $= " ")
		%payload = "";

	$WZClans::Community::Browser::Active = 1;
	$WZClans::Community::Browser::GotResponse = 0;

	wzClansHttpRequest("browser", %path, %payload);
}


// URL escape a string
function wzClans_browser_urlescape(%string)
{
...
function wzClans_browser_request_cert()
{
	error("Browser: Downloading enhanced certificate from community server.");
	wzClans_browser_processRequest("method=cert");
}
...
function wzClans_browser_clan_search(%name)
{
	$Browser::CCount = "";
	%name = wzClans_browser_urlescape(%name);
	wzClans_browser_processRequest("method=csearch&a0=" @ %name);
}

// views clan information for a given clan (e.g. memberships, info, website, recruiting, etc.)
function wzClans_browser_clan_view(%id)
{
	wzClans_browser_processRequest("method=cview&a0=" @ %id);
}

// views clan history (e.g. renamings, retaggings, invitations, kickings, ranking)
function wzClans_browser_clan_history(%id)
{
	wzClans_browser_processRequest("method=chist&a0=" @ %id);
}
...
function wzClans_browser_clan_recruiting(%id, %set)
{
	wzClans_browser_processRequest("method=crecr&a0=" @ %id @ "&a1=" @ %set);
}

// set the clan info field string (alteration rank)
function wzClans_browser_clan_info(%id, %info)
{
	...
	wzClans_browser_processRequest("", %header @ %payload);
}

// set the clan tag, and whether it is prepended or appended to player names (alteration rank)
function wzClans_browser_clan_retag(%id, %tag, %append)
{
	%tag = wzClans_browser_urlescape(%tag);
	wzClans_browser_processRequest("method=ctag&a0=" @ %id @ "&a1=" @ %tag @ "&a2=" @ %append);
}

// set the clan website string (alteration rank)
function wzClans_browser_clan_website(%id, %website)
{
	%website = wzClans_browser_urlescape(%website);
	wzClans_browser_processRequest("method=csite&a0=" @ %id @ "&a1=" @ %website);
}

// set the clan name string (alteration rank)
function wzClans_browser_clan_rename(%id, %name)
{
	%name = wzClans_browser_urlescape(%name);
	wzClans_browser_processRequest("method=cname&a0=" @ %id @ "&a1=" @ %name);
}

// set the filename of the displayed clan picture in the in-game browser (alteration rank)
function wzClans_browser_clan_picture(%id, %picture)
{
	%picture = wzClans_browser_urlescape(%picture);
	wzClans_browser_processRequest("method=cpict&a0=" @ %id @ "&a1=" @ %picture);
}

// invite another player (by GUID) to this clan (recruitment rank)
function wzClans_browser_clan_sendInvite(%id, %invitee)
{
	wzClans_browser_processRequest("method=cinvite&a0=" @ %id @ "&a1=" @ %invitee);
}

// cancel another player's (by GUID) invitation to this clan (recruitment rank, for self invites -- admin for any invite)
function wzClans_browser_clan_retractInvite(%id, %invitee)
{
	wzClans_browser_processRequest("method=cretract&a0=" @ %id @ "&a1=" @ %invitee);
}

// set a list of outstanding invitations for a clan (recruitment rank)
function wzClans_browser_clan_viewInvites(%id)
{
	wzClans_browser_processRequest("method=cinvview&a0=" @ %id);
}
...
function wzClans_browser_clan_changeRank(%id, %target, %rank, %title)
{
	%title = wzClans_browser_urlescape(%title);
	wzClans_browser_processRequest("method=crank&a0=" @ %id @ "&a1=" @ %target @ "&a2=" @ %rank @ "&a3=" @ %title);
}
...
function wzClans_browser_clan_disband(%id, %set)
{
	wzClans_browser_processRequest("method=cdisb&a0=" @ %id @ "&a1=" @ %set);
}


// kick a user from the clan
// requires alteration rank, but cannot kick users at same (or greater) level
function wzClans_browser_clan_kick(%id, %target)
{
	wzClans_browser_processRequest("method=ckick&a0=" @ %id @ "&a1=" @ %target);
}
...
function wzClans_browser_user_search(%name)
{
	%name = wzClans_browser_urlescape(%name);
	$Browser::PCount = "";
	wzClans_browser_processRequest("method=usearch&a0=" @ %name);
}

// view info for a user -- profile, clan memberships, registration date, etc.
function wzClans_browser_user_view(%guid)
{
	wzClans_browser_processRequest("method=uview&a0=" @ %guid);
}

// view history for a user -- clan creations/joins/leavings/kicks/kicked/disband/renamings
function wzClans_browser_user_history(%guid)
{
	wzClans_browser_processRequest("method=uhist&a0=" @ %guid);
}
...
function wzClans_browser_user_rename(%name)
{
	%name = wzClans_browser_urlescape(%name);
	wzClans_browser_processRequest("method=uname&a0=" @ %name);
}
...
function wzClans_browser_user_activeClan(%id)
{
	wzClans_browser_processRequest("method=uclan&a0=" @ %id);
}

// set profile website link
function wzClans_browser_user_website(%website)
{
	%website = wzClans_browser_urlescape(%website);
	wzClans_browser_processRequest("method=usite&a0=" @ %website);
}

// set the filename of the displayed user picture in the in-game browser
function wzClans_browser_user_picture(%picture)
{
	%picture = wzClans_browser_urlescape(%picture);
	wzClans_browser_processRequest("method=upict&a0=" @ %picture);
}

// set info string to given data
function wzClans_browser_user_info(%info)
{
	%guid = getField(wzClansGetLoginCertificate(), 1);
	%uuid = $WZClans::Community::UUID;
...
function wzClans_browser_user_acceptInvite(%id)
{
	wzClans_browser_processRequest("method=uaccept&a0=" @ %id);
}

// ditto to above, but rejects the invite instead
function wzClans_browser_user_rejectInvite(%id)
{
	wzClans_browser_processRequest("method=ureject&a0=" @ %id);
}
...
function wzClans_browser_user_leaveClan(%id)
{
	wzClans_browser_processRequest("method=uleave&a0=" @ %id);
}

// create a clan -- specify the tag/name/info
function wzClans_browser_user_createClan(%tag, %append, %name, %info, %recruiting)
{
...
// schedule(3000, 0, wzClans_Browser_request_cert);
