...
//  - Adapted from the TribesNext t2-scripts browserUI.cs (TribesNext Project, 2012-2013).
...
//    Stock Dynamix GUI function names (SearchWarriors, TWBTabView::view,
//    TribePropertiesDlg::*, PlayerPane::*, GuiMLTextCtrl::onURL, etc.) are intentionally
//    NOT renamed -- this script hooks the stock UI by redefining those functions, so the
//    stock scripts/webbrowser.cs must have been loaded BEFORE this file is exec'd.
//  - No logic changes beyond the renames; this is a verbatim port otherwise.
//  - STOCK GUI DEPENDENCIES (must exist on the client; this file touches them without
//    isObject guards, exactly as the original did): the stock TribeAndWarriorBrowserGui
//    controls TL_Roster, TL_News, TL_Invites, TW_Admin, TL_Profile, TP_NewTag,
//    TP_TribeTagBtn, ProfileControl, TribePropertiesDlg, tb_onProbation, tb_tribeMember,
//    tb_tribeAdmin, tb_tribeController, tb_sysAdmin, plus LaunchTabView, TWBTabView,
//    CreateTribeDlg, PlayerPane, WarriorPopup/WarriorPopupDlg, PlayerGraphic and Canvas.
//    If a client ships a stripped-down GUI set, wzClans_browserui_modifyUIElements()
//    (invoked at exec time, below) will spam "unknown object" console errors.

// redownload a clan profile only if at least this amount of time has passed since the last download
$WZClans::Community::BrowserUI::MinRefreshTime = 30000;

// modify a few user interface elements to match some of the data rearrangements
function wzClans_browserui_modifyUIElements()
{
...
wzClans_browserui_modifyUIElements();

// =========================================================================
// User interface update hook to data interface.
// =========================================================================
function wzClans_browserui_clearCheckStatus()
{
	if (isEventPending($WZClans::Community::BrowserUI::StatusSchedule))
		cancel($WZClans::Community::BrowserUI::StatusSchedule);

	if ($WZClans::Community::Browser::Active)
	{
		$WZClans::Community::BrowserUI::StatusSchedule = schedule(32, 0, wzClans_browserui_clearCheckStatus);
		return;
	}
	...
	if (TribeAndWarriorBrowserGui.searchActive)
	{
		...
		wzClans_browserui_displaySearchResults();
	}
	if (TribePane.updateActive)
	{
		TribePane.updateActive = 0;
		wzClans_browserui_showTribePane();
	}
	if (PlayerPane.updateActive)
	{
		PlayerPane.updateActive = 0;
		wzClans_browserui_showPlayerPane();
	}
...
function wzClans_browserui_displaySearchResults()
{
	if (BrowserSearchPane.query $= "clan")
	{
...
function GuiMLTextCtrl::onURL(%this, %url)
{
	...
	switch$(%fld[0])
	{
		...
		case "retract": // TribesNext version
			%clan = wzClans_browser_getClanProfile(%fld[1]);
			%target = %fld[2];
			%player = wzClans_browser_getPlayerProfile(%target);
			%tname = %player.name;
			if (%tname $= "")
				%tname = %fld[3];
			...
			MessageBoxYesNo("RETRACT", "Are you sure you wish to retract\n<spush><color:FFBB33>" @ %tname @ "<spop>'s invite to\n\"<spush><color:FFBB33>" @ %clan.name @ "<spop>\"?",
				"wzClans_browser_clan_retractInvite(" @ expandEscape(%fld[1]) @ ", " @ expandEscape(%target) @ ");", "");
		...
		case "acceptinvite": // TribesNext version
			...
			MessageBoxYesNo("INVITATION", "Accept invitation to join\n\"<spush><color:FFBB33>" @ %cname @ "<spop>\"?",
				"wzClans_browser_user_acceptInvite(" @ expandEscape(%fld[1]) @ ");", "");
		...
		case "rejectinvite": // TribesNext version
			...
			MessageBoxYesNo("INVITATION", "Are you sure you want to REJECT invitation to join\n\"<spush><color:FFBB33>" @ %cname @ "<spop>\"?",
				"wzClans_browser_user_rejectInvite(" @ expandEscape(%fld[1]) @ ");", "");
		...
		case "invite": // TribesNext version
			...
			MessageBoxYesNo("RETRACT", "Are you sure you wish to invite\n<spush><color:FFBB33>" @ %tname @ "<spop> to join\n\"<spush><color:FFBB33>" @ %clan.name @ "<spop>\"?",
				"wzClans_browser_clan_sendInvite(" @ expandEscape(%fld[1]) @ ", " @ expandEscape(%target) @ ");", "");

		case "addBuddy": // TribesNext Version
			MessageBoxYesNo("CONFIRM","Add \"" @ %fld[2] @ "\" to Buddy List?",
				"wzClans_mail_request_addListEntry(\"buddy\", \"" @ expandEscape(%fld[1]) @ "\"); PlayerPane.updateActive = 1; schedule(300, 0, wzClans_browserui_clearCheckStatus);","");
		case "delBuddy": // TribesNext Version
			MessageBoxYesNo("CONFIRM","Remove \"" @ %fld[2] @ "\" from Buddy List?",
				"wzClans_mail_request_delListEntry(\"buddy\", \"" @ expandEscape(%fld[1]) @ "\"); PlayerPane.updateActive = 1; schedule(300, 0, wzClans_browserui_clearCheckStatus);","");
		...
		case "activeclan": // TribesNext version
			...
			MessageBoxYesNo("CONFIRM", "Are you sure you wish to set \n\"<spush><color:FFBB33>" @ %clan.name @ "<spop>\"\n as your active clan?",
				"wzClans_browser_user_activeClan(" @ expandEscape(%fld[1]) @ "); PlayerPane.updateActive = 1; wzClans_browserui_clearCheckStatus();","");

		case "leaveclan": // TribesNext version
			%clan = wzClans_browser_getClanProfile(%fld[1]);
			MessageBoxYesNo("CONFIRM", "Are you sure you wish to leave \n\"<spush><color:FFBB33>" @ %clan.name @ "<spop>\"?",
				"wzClans_browser_user_leaveClan(" @ expandEscape(%fld[1]) @ "); PlayerPane.updateActive = 1; wzClans_browserui_clearCheckStatus();","");
...
function TribePane::ButtonClick(%this, %senderid)
{
	...
	%now = getSimTime();
	if (%now < $WZClans::Community::BrowserUI::MinRefreshTime)
		%now = ($WZClans::Community::BrowserUI::MinRefreshTime + 1);

	switch(%senderid)
	{
		case 0: //PROFILE
			%this.state = "VIEW_CLAN";
			%clanobj = wzClans_browser_getClanProfile(%clanid);
			%delta = %now - %clanobj.lastRefresh;

			if ($WZClans::Community::BrowserUI::MinRefreshTime < %delta)
			{
				%this.updateActive = 1;
				wzClans_browser_clan_view(%clanid);
				wzClans_browserui_clearCheckStatus();
			}
			else
			{
		...
		case 1: //Formerly "Roster", now "History"
			...
			if ($WZClans::Community::BrowserUI::MinRefreshTime < %delta)
			{
				...
				wzClans_browser_clan_history(%clanid);
				wzClans_browserui_clearCheckStatus();
			}
			else
			{
		...
		case 3: //INVITE BUTTON
			...
			wzClans_browser_clan_viewInvites(%clanid);
			wzClans_browserui_clearCheckStatus();
		case 4: //Admin Tribe
...
function wzClans_browserui_amIMember(%clanid)
{
	return (wzClans_browserui_myRankIn(%clanid) >= 0);
}

function wzClans_browserui_myRankIn(%clanid)
{
...
function wzClans_browserui_showTribePane()
{
	%this = TribePane;
	...
	%isMember = wzClans_browserui_amIMember(%clanid);
	%myRank = wzClans_browserui_myRankIn(%clanid);
			...
			if (%hcount !$= "")
			{
				for (%i = %hcount; %i >= 0; %i--)
				{
					if (%clan.historyCache[%i] $= "")
					{
						...
						%etime = wzClans_mailui_epochToDate(getField(%event, 1));
						%payload = collapseEscape(getField(%event, 2));
						%template = getField(%event, 3);
						%player1 = getFields(%event, 4, 7);
						%player2 = getFields(%event, 8);
						%expanded = wzClans_browserui_expandTemplate(%template, %payload, %player1, %player2);
		...
		case "CLAN_INVITES":
			...
			if (%icount !$= "")
			{
				...
				for (%i = 0; %i <= %icount; %i++)
				{
					...
					%expiration = wzClans_mailui_epochToDate(getField(%invite, 0));
				...
				for (%i = 0; %i <= %icount; %i++)
				{
					...
					%sender = %sPad @ wzClans_browserui_liveLinkPlayer(%sender);
					%recipientguid = getField(%recipient, 3);
					%recipient = %rPad @ wzClans_browserui_liveLinkPlayer(%recipient);
...
function wzClans_browserui_liveLinkPlayer(%player)
{
...
function wzClans_browserui_liveLinkClan(%clan)
{
	%id = getField(%clan, 0);
...
function wzClans_browserui_expandTemplate(%template, %payload, %player1, %player2, %player, %clan)
{
	%pre = "<spush><color:ADFFAD>";
	%val["@player"] = wzClans_browserui_liveLinkPlayer(%player);
	%val["@clan"] = wzClans_browserui_liveLinkClan(%clan);
	%val["@player1"] = wzClans_browserui_liveLinkPlayer(%player1);
	%val["@player2"] = wzClans_browserui_liveLinkPlayer(%player2);
...
function TribePropertiesDlg::RenameTribe(%this)
{
	...
	else
	{
		...
		MessageBoxYesNo("CONFIRM", "Are you sure you want to change the clan name from \n\"<spush><color:FFBB33>" @ %org @ "<spop>\"\nto\n\"<spush><color:FFBB33>" @ %new @ "<spop>\"?",
			"wzClans_browser_clan_rename(\"" @ expandEscape(%clanid) @ "\", \"" @ expandEscape(%new) @ "\");","");
	}
}
...
function TribePropertiesDlg::DisbandTribe(%this)
{
	...
	MessageBoxYesNo("AUTHORIZE","At least 50% of Rank 4 members must authorize a disband." NL " " NL
		...
		"wzClans_browser_clan_disband(\"" @ expandEscape(%clanid) @ "\", 1);",
		"wzClans_browser_clan_disband(\"" @ expandEscape(%clanid) @ "\", 0);");
}
...
function TribePropertiesDlg::ChangeRecruiting(%this)
{
	...
	if (%recru != %recruiting)
	{
		...
		wzClans_browser_clan_recruiting(%clanid, %recruiting);
...
function TribePropertiesDlg::ChangeTag(%this)
{
	...
	if(TP_NewTag.getValue() !$= "")
	{
		...
		wzClans_browser_clan_retag(%clanid, %tag, %append);
	}
	else
	{
...
function TribePropertiesDlg::setTribeGraphic(%this)
{
	...
	%clanid = TribePane.targetid;
	%clan = wzClans_browser_getClanProfile(%clanid);
	wzClans_browser_clan_picture(%clanid, %picture);
}
...
function TribePropertiesDlg::ClearDescription(%this)
{
	...
	MessageBoxYesNo("DESCRIPTION","Are you sure you want to clear the clan info description?",
		"wzClans_browser_clan_info(" @ %clanid @ ", \"\");TP_TribeDescription.setText(\"\");","");
}
...
function EditDescriptionApply()
{
	...
	if(TWBText.editType $= "tribe")
	{
		...
		wzClans_browser_clan_info(%clanid, %desc);
	}
	else
	{
		wzClans_browser_user_info(%desc);
...
function TribeMemberPopupDlg::onWake(%this)
{
	...
	%isMember = wzClans_browserui_amIMember(%tclanid);
	...
	if(%isMember)
	{
		...
		%myRank = wzClans_browserui_myRankIn(%tclanid);
...
function TribeMemberPopup::onSelect(%this, %id, %text)
{
	...
	%trank = getField(%trow, 2);
	%tclanid = getField(%trow, 3);
	%clan = wzClans_browser_getClanProfile(%tclanid);
	...
	switch( %id )
	{
		case 0: // Kick
			if (%tguid != %myguid)
			{
				MessageBoxYesNo("CONFIRM", "Are you sure you want to kick \"<spush><color:FFBB33>\n" @ %tname @ "<spop>\" from \"<spush><color:FFBB33>" @ %clan.name @ "<spop>\"?",
					"wzClans_browser_clan_kick(\"" @ expandEscape(%tclanid) @ "\", \"" @ expandEscape(%tguid) @ "\");", "");
			}
			else
			{
				...
				MessageBoxYesNo("CONFIRM", "Are you sure that you want to leave \"<spush><color:FFBB33>" @ %clan.name @ "<spop>\"?",
					"wzClans_browser_user_leaveClan(\"" @ expandEscape(%tclanid) @ "\");", "");
			}
		case 1: // Admin Member
		...
		case 3: // Add To Buddylist
			MessageBoxYesNo("CONFIRM","Add \"" @ %tname @ "\" to Buddy List?",
				"wzClans_mail_request_addListEntry(\"buddy\", \"" @ expandEscape(%tguid) @ "\");","");
		case 4: // Add To Blocklist
			MessageBoxYesNo("CONFIRM","Block Email from \"" @ %tname @ "\"?",
				"wzClans_mail_request_addListEntry(\"ignore\", \"" @ expandEscape(%tguid) @ "\");","");
	}
...
function SetMemberProfile()
{
	if(strLen(trim(E_Title.getValue)) <= 0)
	{
		...
		wzClans_browser_clan_changeRank(%clanid, %guid, %rank, %title);
...
{  
	...
	switch(%senderid)
	{
		case 0: // Player Profile
			...
			%playerobj = wzClans_browser_getPlayerProfile(%guid);
			%delta = %now - %playerobj.lastRefresh;

			if ($WZClans::Community::BrowserUI::MinRefreshTime < %delta)
			{
				%this.updateActive = 1;
				wzClans_browser_user_view(%guid);
				wzClans_browserui_clearCheckStatus();
			}
			else
			{
		...
		case 1:
			//Player History
                        ...
                        if ($WZClans::Community::BrowserUI::MinRefreshTime < %delta)
                        {
                                ...
                                wzClans_browser_user_history(%guid);
                                wzClans_browserui_clearCheckStatus();
                        }
                        else
                        {
...
function wzClans_browserui_showPlayerPane()
{
	%this = PlayerPane;
	%myguid = getField(WONGetAuthInfo(), 3);
	%guid = %this.targetid;
	...
	switch$ (%this.state)
	{
			...
			if (%hcount !$= "")
			{
				for (%i = 0; %i <= %hcount; %i++)
				{
					if (%player.historyCache[%i] $= "")
					{
						...
						%expanded = wzClans_browserui_expandTemplate(%template, %payload, "", "", %playerv, %clan);
		...
		case "PLAYER_OPTIONS":
			...
			%self = wzClans_browser_getPlayerProfile(%myguid);
			if (%self.mcount !$= "")
			{
				%idxs = "";
	...
	switch( %id )
	{
		case 0: // set active clan tag to none (but retain membership)
			MessageBoxYesNo("CONFIRM", "Are you sure you wish to hide your tag?",
				"wzClans_browser_user_activeClan(-1); PlayerPane.updateActive = 1; wzClans_browserui_clearCheckStatus();","");

		case 1: // set active clan tag to one of the clans the user is a member of
			MessageBoxYesNo("CONFIRM", "Are you sure you wish to set \n\"<spush><color:FFBB33>" @ %this.text @ "<spop>\"\n as your active clan?",
				"wzClans_browser_user_activeClan(" @ expandEscape(%this.id) @ "); PlayerPane.updateActive = 1; wzClans_browserui_clearCheckStatus();","");

		case 2: // leave Tribe
			MessageBoxYesNo("CONFIRM", "Are you sure you wish to leave \n\"<spush><color:FFBB33>" @ %this.text @ "<spop>\"?",
				"wzClans_browser_user_leaveClan(" @ expandEscape(%this.id) @ "); PlayerPane.updateActive = 1; wzClans_browserui_clearCheckStatus();","");
		...
		case 4: // Remove Buddy
			MessageBoxYesNo("CONFIRM","Remove \"" @ WarriorPopup.text @ "\" from Buddy List?",
				"wzClans_mail_request_delListEntry(\"buddy\", \"" @ expandEscape(WarriorPopup.id) @ "\"); PlayerPane.updateActive = 1; schedule(300, 0, wzClans_browserui_clearCheckStatus);","");
...
function WarriorPropertiesDlg::setPlayerGraphic(%this)
{
	...
	wzClans_browser_user_picture(PlayerGraphic.bitmap);
...
function WarriorPropertiesDlg::setURL(%this)
{
	...
	wzClans_browser_user_website(%url);
...
function WarriorPropertiesDlg::ProcessNameChange(%this)
{
	...
	wzClans_browser_user_rename(%name);
...
function CreateTribeProcess()
{
	...
	wzClans_browser_user_createClan(%tag, %append, %name, %info, %recru);
}
